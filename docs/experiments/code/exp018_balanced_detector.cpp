...
//
// Constants come from model_bal/model_meta.json via tools/emit_params.py
...
// --- model constants -------------------------------------------------------
// All generated into kws_params.h from model_bal/model_meta.json.
...
// Operating point swept on FRESH controlled live captures (EXP-018), using the
// balanced model. Measured Pareto frontier -- nothing beats these on both axes:
//
//   thr 0.50  1-of-1  ->  8/10 detected, 7.43 false/min
//   thr 0.50  2-of-3  ->  5/10 detected, 2.97 false/min
//   thr 0.70  2-of-2  ->  4/10 detected, 0.00 false/min   <-- chosen
//
// 0.70 with 2-of-2 is chosen for demonstration: a device that never fires
// unprompted and occasionally needs the word repeated is usable, while one that
// fires seven times a minute is not. Change DETECT_THRESHOLD to 0.50 and
// DETECT_M/N to 1 for the 80 %%-detection point if false fires matter less.
//
// CAVEAT: tuned on the same 80 s it was validated against. Provisional.
constexpr float DETECT_THRESHOLD = 0.70f;
constexpr int DETECT_M = 2;
constexpr int DETECT_N = 2;
