...
constexpr int N_FRAMES = 98;        // 1.0 s context
constexpr float MEL_LO = 125.0f, MEL_HI = 7500.0f;

// --- model constants (model/model_meta.json) --------------------------------
constexpr float NORM_MEAN = 15.813641548156738f;
constexpr float NORM_STD = 2.5956406593322754f;
constexpr float IN_SCALE = 0.02723478339612484f;
constexpr int IN_ZP = -15;
constexpr float OUT_SCALE = 0.0741243064403534f;
constexpr int OUT_ZP = -14;
...
constexpr int kArenaSize = 145 * 1024;
...
float g_feat[N_FRAMES][N_MEL];      // rolling feature buffer
int g_feat_head = 0;
...
void runInference() {
  ...
  for (int f = 0; f < N_FRAMES; ++f) {
    ...
    for (int m = 0; m < N_MEL; ++m) {
      const float norm = (row[m] - NORM_MEAN) / NORM_STD;
  ...
  // Feature cost recurs every 10 ms; inference every INFER_EVERY_HOPS * 10 ms.
  const double cpu = 100.0 * (feat_ms / 10.0 + inf_ms / (10.0 * INFER_EVERY_HOPS));
...
void setup() {
  ...
  for (int f = 0; f < N_FRAMES; ++f)
    for (int m = 0; m < N_MEL; ++m) g_feat[f][m] = NORM_MEAN;
