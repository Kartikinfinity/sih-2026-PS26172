...
//
// Constants come from model_aug/model_meta.json via tools/emit_params.py
...
// --- model constants -------------------------------------------------------
// All generated into kws_params.h from model_aug/model_meta.json.
...
// Operating point swept on the two CONTROLLED LIVE captures (EXP-017) -- real
// deployment audio, not session hold-out, which had been optimistic twice.
//
//   thr 0.50  2-of-3  ->  2/10 detected, 0.00 false/min
//   thr 0.40  1-of-1  ->  4/10 detected, 0.00 false/min   <-- chosen
//   thr 0.30  2-of-3  ->  5/10 detected, 1.49 false/min
//   thr 0.30  1-of-1  ->  6/10 detected, 4.46 false/min
//
// Smoothing is set to 1-of-1 (off) because after the Speech Commands import the
// model's keyword responses became weak but its false responses became weaker
// still: measured peak on 40 s of non-keyword speech was 0.351, against
// keyword peaks reaching 0.776. Separation now lives in the tail, so a lower
// threshold recovers detections while the margin to 0.351 suppresses false
// fires. Voting on top of that only discards real detections.
//
// CAVEAT: 0.40 is tuned on 40 s of audio. The margin to the observed negative
// maximum (0.351) is thin and this figure is provisional until re-measured on
// fresh audio.
constexpr float DETECT_THRESHOLD = 0.40f;
constexpr int DETECT_M = 1;
constexpr int DETECT_N = 1;
